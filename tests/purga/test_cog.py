...
from discord_bot.purga.cog import BUTTON_STYLES, COG_NAME, PURGA_CONFIG_SCHEMA, PurgaCog
...
class TestFormatMessage:
    ...
    def test_format_all_placeholders(self, purga_cog: PurgaCog) -> None:
        """Probar reemplazo de todos los placeholders."""
        template = "Estado: {status}, Dia: {dia}"

        result = purga_cog._format_message(
            template,
            status="Pendiente",
            dia="2024-01-01",
        )
    ...
    def test_format_empty_placeholders(self, purga_cog: PurgaCog) -> None:
        """Probar con placeholder pasado como None."""
        template = "Usuario: {username}"
        result = purga_cog._format_message(template, username=None)
        assert result == "Usuario: "

    def test_format_unmatched_placeholder(self, purga_cog: PurgaCog) -> None:
        """Probar que placeholders no pasados se mantienen."""
        template = "Usuario: {username}"
        result = purga_cog._format_message(template)
        assert result == "Usuario: {username}"

    def test_format_no_placeholders(self, purga_cog: PurgaCog) -> None:
        """Probar mensaje sin placeholders."""
        template = "Mensaje simple sin placeholders"
        result = purga_cog._format_message(template)
        assert result == template

    def test_format_none_template(self, purga_cog: PurgaCog) -> None:
        """Probar con template None."""
        result = purga_cog._format_message(None, status="Test")
        assert result == ""


class TestGetButtonStyle:
    """Tests para _get_button_style."""

    def test_blurple(self, purga_cog: PurgaCog) -> None:
        """Probar estilo blurple."""
        result = purga_cog._get_button_style("blurple")
        assert result == discord.ButtonStyle.primary

    def test_grey(self, purga_cog: PurgaCog) -> None:
        """Probar estilo grey."""
        result = purga_cog._get_button_style("grey")
        assert result == discord.ButtonStyle.secondary

    def test_green(self, purga_cog: PurgaCog) -> None:
        """Probar estilo green."""
        result = purga_cog._get_button_style("green")
        assert result == discord.ButtonStyle.success

    def test_red(self, purga_cog: PurgaCog) -> None:
        """Probar estilo red."""
        result = purga_cog._get_button_style("red")
        assert result == discord.ButtonStyle.danger

    def test_unknown_defaults_to_success(self, purga_cog: PurgaCog) -> None:
        """Probar que color desconocido devuelve success."""
        result = purga_cog._get_button_style("unknown")
        assert result == discord.ButtonStyle.success


class TestFormatAuthorizedBy:
    """Tests para _format_authorized_by."""

    def test_empty_list(self, purga_cog: PurgaCog, mock_guild: MagicMock) -> None:
        """Probar con lista vacia."""
        result = purga_cog._format_authorized_by(mock_guild, [])
        assert result == "Ninguno"

    def test_with_found_members(self, purga_cog: PurgaCog, mock_guild: MagicMock) -> None:
        ...
        result = purga_cog._format_authorized_by(mock_guild, [111, 222])
        assert result == "User1, User2"

    def test_with_not_found_members(self, purga_cog: PurgaCog, mock_guild: MagicMock) -> None:
    ...
    def test_mixed_members(self, purga_cog: PurgaCog, mock_guild: MagicMock) -> None:
...
class TestFormatRoles:
    """Tests para _format_roles."""

    def test_empty_list(self, purga_cog: PurgaCog, mock_guild: MagicMock) -> None:
        """Probar con lista vacia."""
        result = purga_cog._format_roles(mock_guild, [])
        assert result == "Ninguno"

    def test_with_found_roles(self, purga_cog: PurgaCog, mock_guild: MagicMock) -> None:
        ...
        result = purga_cog._format_roles(mock_guild, [100, 200])
        assert result == "@Role1, @Role2"

    def test_with_not_found_roles(self, purga_cog: PurgaCog, mock_guild: MagicMock) -> None:
...
class TestGetModMessageContent:
    """Tests para _get_mod_message_content."""
    ...
    def test_pending_status(
        ...
        result = purga_cog._get_mod_message_content(
...
class TestSetupAndTeardown:
    ...
    async def test_setup_registers_schema_and_adds_cog(self, mock_discord_bot: MagicMock) -> None:
        ...
        from discord_bot.purga.cog import PURGA_CONFIG_SCHEMA, setup
...
class TestButtonStyles:
    """Tests para constante BUTTON_STYLES."""

    def test_all_button_styles_defined(self) -> None:
        """Probar que todos los estilos de boton estan definidos."""
        assert "blurple" in BUTTON_STYLES
        assert "grey" in BUTTON_STYLES
        assert "green" in BUTTON_STYLES
        assert "red" in BUTTON_STYLES

    def test_button_styles_are_valid(self) -> None:
        """Probar que los estilos son validos."""
...
class TestMaintenancePurgeType:
    """Tests para tipo de purga de mantenimiento."""

    def test_maintenance_purge_type_label(
        self,
        purga_cog: PurgaCog,
        mock_guild: MagicMock,
        mock_purga_record: MagicMock,
    ) -> None:
        """Probar etiqueta de purga de mantenimiento."""
...
class TestAllStatusMessages:
    """Tests para todos los estados de mensaje."""
    ...
    def test_expired_status(
        ...
        """Probar estado expirado."""
        mock_purga_record.status = PurgaStatus.EXPIRED
        config: dict[str, Any] = {
            ConfigKey.MOD_MESSAGE_TEMPLATE: "Estado: {status}",
            ConfigKey.MOD_STATUS_EXPIRED: "Expirado",
            ConfigKey.MOD_REQUIRED_REACTIONS: 2,
        }
    ...
    def test_cancel_pending_status(
        self,
        purga_cog: PurgaCog,
        mock_guild: MagicMock,
        mock_purga_record: MagicMock,
    ) -> None:
        """Probar estado cancelacion pendiente."""
        mock_purga_record.status = PurgaStatus.CANCEL_PENDING
        config: dict[str, Any] = {
            ConfigKey.MOD_MESSAGE_TEMPLATE: "Estado: {status}",
            ConfigKey.MOD_STATUS_CANCEL_PENDING: "Cancelacion pendiente",
            ConfigKey.MOD_REQUIRED_REACTIONS: 2,
        }
        ...
        assert "Cancelacion pendiente" in result
    ...
    def test_cancelled_status(
        ...
        """Probar estado cancelado."""
        mock_purga_record.status = PurgaStatus.CANCELLED
        config: dict[str, Any] = {
            ConfigKey.MOD_MESSAGE_TEMPLATE: "Estado: {status}",
    ...
    def test_executed_status(
        ...
        """Probar estado ejecutado."""
        mock_purga_record.status = PurgaStatus.EXECUTED
        config: dict[str, Any] = {
            ConfigKey.MOD_MESSAGE_TEMPLATE: "Estado: {status}",
            ConfigKey.MOD_STATUS_EXECUTED: "Ejecutado",
            ConfigKey.MOD_REQUIRED_REACTIONS: 2,
        }
